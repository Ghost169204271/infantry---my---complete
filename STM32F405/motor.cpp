#include"motor.h"
Motor::Motor(const type_t type, const motor_mode mode, const motor_use use, const uint32_t id, PID _speed, PID _position): ID(id), type(type), mode(mode),m_motor_use(use)
{
	getmax(type);
	memcpy(&pid[speed], &_speed, sizeof(PID));
	memcpy(&pid[position], &_position, sizeof(PID));
}
Motor::Motor(const type_t type, const motor_mode mode, const motor_use use, const uint32_t id, PID _speed): ID(id), type(type), mode(mode), m_motor_use(use)
{
	getmax(type);
	memcpy(&pid[speed], &_speed, sizeof(PID));
}
void Motor::Ontimer(uint8_t idata[][8], uint8_t* odata)//idate: receive;odate: trainsmit;RC
{
	...
	if (mode == ACE)
	{
			...
			if(pd)
		{
			...
			current += pid[speed].Delta(setspeed - curspeed);
			current = setrange(current, maxcurrent);
			uint8_t deviation = 150;
			uint16_t lastStopMin = stopAngle - deviation, lastStopMax=stopAngle + deviation;
			if (angle[LAST] > lastStopMin && angle[now] < lastStopMax)
			{
				curcircle++;
			}
			if(curcircle>4||curcircle<0)
			{
				curcircle = 0;
				stopAngle = angle[now];
				pd = 0;
	...
	{
		//if (setangle - angle[now] >= 5 || setangle - angle[now] <= -5)
		{
			setspeed = pid[position].Position(kalman.Filter(getdeltaa(setangle - angle[now])));
		//	setspeed = pid[position].Position(getdeltaa(setangle - angle[now]));
