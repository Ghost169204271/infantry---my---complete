...
public:
	...
	uint32_t id_blue = 0x0167;
	uint32_t id_red = 0x0103;
	...
	void OnIRQHandler(size_t readnum);
	void GetData(void);//This function is not called anywhere
	void SendData(void);//This function is not called anywhere
...
private:
	...
	bool transmit(uint32_t read_size, uint8_t* plate);//This function is only called in GetDate, but GetDate is not called anywhere(1)
	void Decode(uint8_t* m_frame);//This function is only called in GetDate, but GetDate is not called anywhere(2)
	void Encode(void);//This function is only called in SendData, but SendData is not called anywhere(3)
	//The following five functions are only called in Encode(4)
	/*(1),(2),(3),(4)
	 * 2022/01/18
	 */
	void Gragh_Top(void);
